from MakimaDevil.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from MakimaDevil import DEVILS
...
async def user_is_admin(user_id: int, message):
    ...
    async for user in telethn.iter_participants(
        ...
        if user_id == user.id or user_id in DEVILS:
            status = True
            break
    return status
...
async def makimatelethon_is_admin(chat_id: int):
    status = False
    makimatelethon = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if makimatelethon.id == user.id:
            status = True
            break
    return status
