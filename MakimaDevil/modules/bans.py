...
from MakimaDevil.modules.helper_funcs.extraction import extract_user_and_text
from MakimaDevil.modules.helper_funcs.string_handling import extract_time
from MakimaDevil.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in KNIGHTS:
        if user_id == OWNER_ID:
            ...
            message.reply_text(
                "Fighting this devils here will put civilian lives at risk."
            )
        elif user_id in ANGELS:
            message.reply_text(
                "Bring an order from Makima's Bf to fight a Angels."
            )
        elif user_id in KILLERS:
            message.reply_text(
                "Bring an order from Makima's Bf to fight a Tiger disaster."
            )
        elif user_id in SENSEI:
...
def selfunban(context: CallbackContext, update: Update) -> str:
    ...
    user = update.effective_user
    bot, args = context.bot, context.args
    if user.id not in DEVILS or user.id not in KILLERS:
        return
