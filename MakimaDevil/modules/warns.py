...
import telegram
from MakimaDevil import KILLERS, SENSEI, dispatcher
...
from MakimaDevil.modules.helper_funcs.misc import split_message
from MakimaDevil.modules.helper_funcs.string_handling import split_quotes
from MakimaDevil.modules.log_channel import loggable
from MakimaDevil.modules.sql import warns_sql as sql
...
from MakimaDevil.modules.sql.approve_sql import is_approved
...
def warn(user: User,
    ...
    if user.id in KILLERS:
        if warner:
            message.reply_text("KILLERS cant be warned.")
        else:
            message.reply_text(
                "Tiger triggered an auto warn filter!\n I can't warn KILLERS but they should avoid abusing this."
            )
        return

    if user.id in SENSEI:
        if warner:
            message.reply_text("Senseis are warn immune.")
        else:
            message.reply_text(
                "Sensei triggered an auto warn filter!\nI can't warn SENSEI but they should avoid abusing this."
            )
        return
