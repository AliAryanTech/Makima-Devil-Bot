...
import MakimaDevil.modules.sql.feds_sql as sql
...
from MakimaDevil.modules.helper_funcs.alternate import send_message
...
from MakimaDevil.modules.helper_funcs.string_handling import markdown_parser
...
def leave_fed(update: Update, context: CallbackContext):
    ...
    if getuser in "creator" or user.id in DEVILS:
...
def user_join_fed(update: Update, context: CallbackContext):
    ...
    if is_user_fed_owner(fed_id, user.id) or user.id in DEVILS:
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if user_id == OWNER_ID:
        message.reply_text("Disaster level Kamisama cannot be fed banned!")
        return

    if int(user_id) in DEVILS:
        message.reply_text("DEVILS cannot be fed banned!")
        return

    if int(user_id) in KILLERS:
        message.reply_text("KILLERS cannot be fed banned!")
        return

    if int(user_id) in SENSEI:
        message.reply_text("SENSEI cannot be fed banned!")
        return
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "makimatelethon_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="makimatelethon_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "makimatelethon_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="makimatelethon_fbanned_users.csv",
            ...
            with BytesIO() as file:
                ...
                for x in splitting:
                    ...
                    if str(import_userid) == str(OWNER_ID):
                        failed += 1
                        continue
                    if int(import_userid) in DEVILS:
                        failed += 1
                        continue
                    if int(import_userid) in KILLERS:
                        failed += 1
                        continue
                    if int(import_userid) in SENSEI:
                        failed += 1
                        continue
...
❍ /feduserhelp*:* Provides help for commands anmakimatelethon can use
