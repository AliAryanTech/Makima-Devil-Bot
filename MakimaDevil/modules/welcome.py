...
from MakimaDevil.modules.helper_funcs.misc import build_keyboard, revert_buttons
from MakimaDevil.modules.helper_funcs.msg_types import get_welcome_type
...
def new_member(update: Update, context: CallbackContext):
    ...
    for new_mem in new_members:
        ...
        if should_welc:
            ...
            # Welcome Sudos
            elif new_mem.id in DEVILS:
                update.effective_message.reply_text(
                    "Whoa! A Devil just joined! Stay Alert!",
                    reply_to_message_id=reply,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
            ...
            # Welcome Support
            elif new_mem.id in ANGELS:
                update.effective_message.reply_text(
                    "Huh! Someone with a Angels level just joined!",
                    reply_to_message_id=reply,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
            ...
            # Welcome KILLERS
            elif new_mem.id in SENSEI:
                update.effective_message.reply_text(
                    "Awoo! A Sensei just joined!", reply_to_message_id=reply
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
            ...
            # Welcome yourself
                ...
                creator = None
                if not MakimaDevil.ALLOW_CHATS:
...
def welcome(update: Update, context: CallbackContext):
    ...
    if not args or args[0].lower() == "noformat":
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            update.effective_message.reply_text(
                "I'll go loaf around and not welcome anmakimatelethon then."
            )
